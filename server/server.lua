...
RegisterNetEvent("insertStashesData", function(input, loc)
    ...
    if not isAdmin(src) then
        print("[mri_Qstashes] - Tentativa não autorizada de criar baú por: " .. GetPlayerName(src) .. " (ID: " .. src .. ")")
        return
    end
...
RegisterNetEvent("deleteStashesData", function(id)
    local src = source
    if not isAdmin(src) then
        print("[mri_Qstashes] - Tentativa não autorizada de deletar baú por: " .. GetPlayerName(src) .. " (ID: " .. src .. ")")
        return
    end
...
RegisterNetEvent("updateStashesData", function(id, input)
    local src = source
    if not isAdmin(src) then
        print("[mri_Qstashes] - Tentativa não autorizada de editar baú por: " .. GetPlayerName(src) .. " (ID: " .. src .. ")")
        return
    end
...
RegisterNetEvent("updateStashLocation", function(id, newLoc)
    local src = source
    if not isAdmin(src) then
        print("[mri_Qstashes] - Tentativa não autorizada de mover baú por: " .. GetPlayerName(src) .. " (ID: " .. src .. ")")
        return
    end
    for i = 1, #stashesTable do
        if stashesTable[i].id == id then
...
AddEventHandler('onResourceStart', function(resourceName)
    if GetCurrentResourceName() == resourceName then
        ...
        -- Aguarda ox_inventory inicializar antes de sincronizar clientes
...
RegisterNetEvent("mri_Qstashes:server:Load", function()
    local src = source
    -- Envia os dados já carregados em memória para o jogador que logou
    -- Sem Wait() dentro de evento de rede para evitar race conditions
