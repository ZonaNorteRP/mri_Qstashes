...
CreateThread(function()
    Wait(2000) -- aguarda o servidor estar pronto
    updateAdminStatus()
end)
...
RegisterNetEvent('mri_Qstashes:start', function(stashesTable)
    -- Limpa zonas antigas antes de recriar
    ...
    for k, v in pairs(stashesTable) do
        ...
        -- Monta as opções do target
        local targetOptions = {
            {
                name = 'openstashes_' .. v.id,
    ...
    print("[mri_Qstashes] - Loaded " .. #stashesTable .. " stashes.")
end)

RegisterNetEvent('mri_Qstashes:delete', function(stashesTable)
    if stashes == nil or next(stashes) == nil then
        return
    end
    for i = 1, #stashes do
        if stashes[i] ~= nil then
